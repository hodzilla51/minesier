...
public final class CryptoApi {
	...
	public static String randomBytes(int count) {
		if (count < 1 || count > MAX_RANDOM_BYTES) throw new IllegalArgumentException("random byte count must be 1-4096");
	...
	public static String hkdfSha256(String input, String salt, String info, int length) {
		if (length < 1 || length > MAX_HKDF_BYTES) throw new IllegalArgumentException("HKDF length is out of range");
	...
	public static String aesGcmDecrypt(String key, String nonce, String ciphertext, String aad) {
		try {
			...
			if (nonceBytes.length != GCM_NONCE_BYTES) throw new IllegalArgumentException("AES-GCM nonce must be 12 bytes");
			Cipher cipher = Cipher.getInstance("AES/GCM/NoPadding");
	...
	public static X25519KeyPair x25519KeyPair() {
		try {
			...
			return new X25519KeyPair(encode(pair.getPrivate().getEncoded()), encode(pair.getPublic().getEncoded()));
		} catch (GeneralSecurityException e) {
	...
	private static byte[] decode(String data) {
		...
		catch (IllegalArgumentException e) { throw new IllegalArgumentException("expected Base64 data", e); }
	}
	private static String encode(byte[] data) { return Base64.getEncoder().encodeToString(data); }
	private static byte[] utf8(String data) { return data.getBytes(StandardCharsets.UTF_8); }
}
