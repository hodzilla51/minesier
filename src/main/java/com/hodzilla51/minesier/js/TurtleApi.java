...
/**
 * The world-facing actions a turtle program can call, exposed to scripts as the
 * {@code turtle} global. Movement returns whether it succeeded; it costs fuel and
 * fails when the tank is empty.
 *
 * <p>Implementations are server-side and authoritative; the VM only ever sees this
 * narrow interface, never Minecraft classes (see the sandbox in {@link JsComputer}).
 */
public interface TurtleApi {
	...
	/** Places the named block ({@code "minecraft:stone"}) ahead, creative-style; false if blocked or unknown. */
	boolean place(String blockId);

	/** Places the block held in the selected inventory slot ahead, consuming one; false if empty/blocked. */
