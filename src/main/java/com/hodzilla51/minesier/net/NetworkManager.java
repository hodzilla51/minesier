 ...
 *
 * <p>Today every caller is on the server thread (computer VMs run synchronously and
 * {@link #dispatch()} runs on the server tick). The queue is still guarded so a
 * future off-thread producer cannot corrupt it. Events run OUTSIDE the lock so a
 * handler that schedules more work can't deadlock or re-enter the queue ops.
 */
