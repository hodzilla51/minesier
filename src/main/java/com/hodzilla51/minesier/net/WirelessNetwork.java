 ...
 *
 * <p>Wireless modems register their position per level (on load) so a transmission
 * doesn't have to scan the world. A frame sent from one modem is offered to the
 * computer attached to every OTHER modem within {@link #RANGE} blocks — like real
 * radio, everyone in range hears it, which is exactly what makes promiscuous
 * wireless sniffing possible.
 */
public final class WirelessNetwork {
	...
	/** Offers {@code frame} to the computer behind every other modem within range of {@code fromModem}. */
