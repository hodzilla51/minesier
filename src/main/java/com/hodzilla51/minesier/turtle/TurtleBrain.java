 ...
 *
 * <p>The JS program runs on a dedicated worker thread. Each {@code turtle.*} call
 * hands an action to the server thread and BLOCKS until {@link #tick()} (driven by
 * the server tick loop) performs it — so movement/dig/place take real time and the
 * program pauses between them, instead of teleporting through everything in one tick.
 *
 * <p>Thread safety: the worker thread only touches the synchronized handoff and its
 * own JS context; ALL world mutation happens on the server thread inside {@link #tick()}.
...
public final class TurtleBrain {
	...
	/** Server thread, once per tick: advances the pending action, performing it when its pacing elapses. */
	public void tick() {
		synchronized (lock) {
	...
	private final TurtleApi proxy = new TurtleApi() {
		@Override
