...
public class MineSIerClient implements ClientModInitializer {
	...
	public void onInitializeClient() {
		...
		ClientPlayNetworking.registerGlobalReceiver(TerminalScreenS2C.TYPE, (payload, context) ->
			context.client().execute(() -> {
				if (payload.open()) {
					context.client().setScreenAndShow(
						new ComputerScreen(payload.pos(), payload.transcript(), payload.turtle()));
				} else {
		...
		ClientPlayNetworking.registerGlobalReceiver(TurtleMoveS2C.TYPE, (payload, context) ->
			context.client().execute(() -> {
		...
		ClientPlayNetworking.registerGlobalReceiver(LoadProgramS2C.TYPE, (payload, context) ->
		...
		ClientPlayNetworking.registerGlobalReceiver(InventoryS2C.TYPE, (payload, context) ->
			context.client().execute(() -> ComputerScreen.showInventory(payload.selected(), payload.slots())));

		// The turtle block is INVISIBLE; this renderer draws it (and slides it on moves).
		BlockEntityRendererRegistry.register(ModContent.TURTLE_BLOCK_ENTITY, TurtleBlockEntityRenderer::new);
	}
}
